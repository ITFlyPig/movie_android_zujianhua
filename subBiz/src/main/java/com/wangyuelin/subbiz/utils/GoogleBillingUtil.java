...
//                        queryPurchasesInApp();
                        ...
                        log("初始化失败:onSetupFail:code=" + (billingResult == null ? -1 : billingResult.getDebugMessage()));
                        if (mOnStartSetupFinishedListener != null) {
                            mOnStartSetupFinishedListener.onSetupFail((billingResult == null ? "返回BillingResult为null" : billingResult.getDebugMessage()));
                        }
                    }
    ...
    private class MyPurchasesUpdatedListener implements PurchasesUpdatedListener {
        ...
        public void onPurchasesUpdated(BillingResult billingResult, @Nullable List<Purchase> list) {
            ...
            if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK && list != null) {//订阅or购买  成功
            ...
            } else if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.USER_CANCELED){//用于取消 订阅or购买  的流程
                mOnPurchaseFinishedListener.onPurchaseFail("用户取消购买");
            } else {
                mOnPurchaseFinishedListener.onPurchaseFail(billingResult.getDebugMessage());
            }
        }
    }
    ...
    public void queryInventoryInApp() {
        queryInventory(BillingClient.SkuType.INAPP, inAppSKUS);
    }
    ...
    public void queryInventorySubs() {
        queryInventory(BillingClient.SkuType.SUBS, subsSKUS);
    }

    private void queryInventory(final String skuType, final String... skus) {
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                ...
                int len = skus == null ? 0 : skus.length;
                if (len <= 0) {
                    return;
                }
    ...
    private class MySkuDetailsResponseListener implements SkuDetailsResponseListener {
        ...
        public void onSkuDetailsResponse(BillingResult billingResult, List<SkuDetails> list) {
            ...
            if (billingResult != null && billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK && list != null) {
                ...
                mOnQueryFinishedListener.onQueryFail(billingResult == null ? "返回BillingResult为null" : billingResult.getDebugMessage());
            }
        }
    }
    ...
    private void purchase(final Activity activity, final String skuId, final String skuType) {
        ...
        if (startConnection()) {
            ...
            mBillingClient.querySkuDetailsAsync(params.build(), new SkuDetailsResponseListener() {//查询商品详情
                @Override
                public void onSkuDetailsResponse(BillingResult billingResult, List<SkuDetails> list) {
                    int len = list == null ? 0 : list.size();
                    if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK && len > 0) {
                        BillingFlowParams flowParams = BillingFlowParams.newBuilder()
                                .setSkuDetails(list.get(0))
                                .build();
                        mBillingClient.launchBillingFlow(activity, flowParams);//购买商品
                    } else {
                        if (mOnPurchaseFinishedListener != null) {
                            mOnPurchaseFinishedListener.onPurchaseError();
                        }
    ...
    private class MyConsumeResponseListener implements ConsumeResponseListener {
        ...
        public void onConsumeResponse(BillingResult billingResult, String s) {
            if (billingResult != null && billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK) {//消耗成功
    ...
    public interface OnPurchaseFinishedListener {
        ...
        public void onPurchaseFail(String errorMsg);
    ...
    public interface OnStartSetupFinishedListener {
        ...
        public void onSetupFail(String errorMsg);
